...
@[ext]
structure Types.Hom (X Y : Type u) where
  private mk ::
...
instance (X Y : Type u) : FunLike (X → Y) X Y where
  coe f := f
  coe_injective' f g := by simp

instance : ConcreteCategory.{u} (Type u) (fun X Y ↦ X → Y) where
  hom := Hom.hom'
  ofHom f := ⟨f⟩

lemma Types.congr_fun {X Y : Type u} {f g : X ⟶ Y} (h : f = g) (x : X) : f x = g x := by
  rw [h]
...
@[ext]
theorem types_ext {α β : Type u} (f g : α ⟶ β) (h : ∀ a : α, f a = g a) : f = g := by
  ext x
  exact h x

theorem types_id (X : Type u) : (𝟙 X : X ⟶ X).hom' = id :=
  rfl

theorem types_comp {X Y Z : Type u} (f : X ⟶ Y) (g : Y ⟶ Z) : (f ≫ g).hom' = g.hom' ∘ f.hom' :=
  rfl
...
@[simp]
theorem hom_inv_id_apply {X Y : Type u} (f : X ≅ Y) (x : X) : f.inv (f.hom x) = x := by
  simp [← types_comp_apply]

@[simp]
theorem inv_hom_id_apply {X Y : Type u} (f : X ≅ Y) (y : Y) : f.hom (f.inv y) = y := by
  simp [← types_comp_apply]
...
def sectionsFunctor : (J ⥤ Type w) ⥤ Type max u w where
  obj F := F.sections
  map {F G} φ := by
    refine ⟨fun x ↦ ⟨fun j => φ.app j (x.1 j), fun {j j'} f => ?_⟩⟩
    rw [← types_comp_apply, ← φ.naturality, types_comp_apply, x.2 f]
